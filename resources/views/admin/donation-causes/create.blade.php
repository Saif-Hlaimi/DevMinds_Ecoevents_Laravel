    ...
    <div class="card-body">
      <form action="{{ route('dashboard.admin.donation-causes.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <input type="number" name="goal_amount" id="goal_amount" class="form-control" step="0.01" min="0.01" value="{{ old('goal_amount') }}" required>
          @error('goal_amount')
            <span class="text-danger small">{{ $message }}</span>
          @enderror
        </div>
        <div class="mb-3">
          ...
          <input type="text" name="sdg" id="sdg" class="form-control" value="{{ old('sdg') }}" required>
          @error('sdg')
            <span class="text-danger small">{{ $message }}</span>
          @enderror
        </div>

    ...
    <div class="card-body">
      <form action="{{ route('dashboard.admin.donation-causes.update', $donationCause->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $donationCause->title) }}" required placeholder="Enter title to generate AI description">
        ...
        <div class="mb-3">
          ...
          <textarea name="description" id="description" class="form-control" rows="5" placeholder="Enter description to generate AI image" required>{{ old('description', $donationCause->description) }}</textarea>
          @error('description')
            <span class="text-danger small">{{ $message }}</span>
          @enderror
        </div>
        <div class="mb-3">
          ...
          <div class="input-group">
            <input type="text" class="form-control" value="{{ $donationCause->image ? basename($donationCause->image) : 'No image selected' }}" readonly id="image-preview-text">
          ...
          <div class="mt-2">
            @if($donationCause->image)
              <img src="{{ asset('storage/' . $donationCause->image) }}" alt="Current Image" class="img-thumbnail mb-2" style="max-width: 200px;">
            @endif
        ...
        <div class="mb-3">
          ...
          <input type="text" list="sdg-list" name="sdg" id="sdg" class="form-control" value="{{ old('sdg', $donationCause->sdg) }}" required placeholder="Sustainable Development Goal (e.g., SDG1)">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (fileInput && fileText) {
    fileInput.addEventListener('change', function () {
      fileText.value = this.files.length > 0 ? this.files[0].name : '{{ $donationCause->image ? basename($donationCause->image) : "No image selected" }}';
